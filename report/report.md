...
Phần còn lại của báo cáo được tổ chức như sau. **Chương 2** trình bày cơ sở lý thuyết, bao gồm phát biểu bài toán, các loại tấn công, các tập dữ liệu benchmark, các nhóm kiến trúc tiếp cận, và phương pháp đánh giá. **Chương 3** mô tả thiết lập thí nghiệm, trình bày kết quả EER tổng hợp trên năm mô hình trong benchmark chính và bốn dataset, đồng thời giải thích trường hợp checkpoint AASIST3 bị loại khỏi phân tích chính. **Chương 4** tổng kết các phát hiện chính, đưa ra đề xuất hướng tiếp cận ở mức ý niệm và các bước phát triển tiếp theo trong khuôn khổ Thực tập 2 và Luận văn.
...
- **F4 — Failure overlap và score correlation gợi ý cơ hội fusion.** Cặp AASIST/AASIST-L overlap 0.62 và correlation 0.84 (cùng họ), trong khi một số cặp khác họ thấp hơn (AASIST với LFCC+LCNN −0.08, LFCC+LCNN với XLS-R+AASIST 0.13). Hai mô hình XLS-R có correlation 0.49 — vẫn còn lỗi riêng dù cùng front-end family. Đây là evidence gián tiếp cho score-level fusion. *(§3.4.5 → Chương 4: fusion/ensemble có kiểm soát.)*
- **F5 — SSL front-end là điều kiện cần nhưng không đủ.** Hai mô hình cùng dùng XLS-R 300M front-end vẫn chênh nhau đáng kể tuỳ dataset: XLS-R + AASIST tốt hơn trên ASV5, còn XLS-R + Nes2Net tốt hơn trên ITW và ASV21 DF. Vì vậy robustness không chỉ đến từ SSL representation, mà còn phụ thuộc back-end, fine-tuning recipe, checkpoint selection và calibration. *(§3.3, §3.4.1, §3.5 → Chương 4: ablation tách đóng góp front-end, back-end và checkpoint recipe.)*
...
- **Clean LA / TTS / VC benchmarks:** ASVspoof 2019 LA, WaveFake, FoR hoặc các dataset lịch sử tương tự. Nhóm này giúp kiểm tra hiệu năng trong điều kiện kiểm soát và tạo baseline so sánh với literature.
...
- **SSL/foundation front-end + anti-spoofing back-end:** wav2vec2, XLS-R, WavLM, HuBERT hoặc Whisper-style encoder kết hợp với AASIST, MFA, Nes2Net hoặc pooling/MLP. Nhóm này hiện là hướng mạnh nhưng chi phí cao và phụ thuộc checkpoint/recipe.
- **Back-end hiện đại:** Nes2Net-LA, XLSR-Mamba, Conformer/Transformer variants, state-space models hoặc local-attention designs. Nhóm này giúp tách vai trò của representation và temporal/back-end modeling.
...
- **Front-end/back-end ablation.** F5 cho thấy SSL front-end mạnh không tự đảm bảo robustness. Một hướng cần làm là giữ cùng front-end nhưng thay back-end, hoặc giữ back-end nhưng thay front-end, để tách đóng góp của representation, temporal modeling và checkpoint recipe.
