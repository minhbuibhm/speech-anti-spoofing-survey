...
**3.4.4 Confident errors.** File `hard_errors.csv` lưu một tập top confident errors từ artifact thô; trong bản export hiện tại, artifact này phù hợp cho qualitative inspection hơn là thống kê định lượng theo từng mô hình (300 dòng đầu có cột model trống và phần này đọc như confident errors của AASIST do cách sort/wide-format). Quan sát định tính cho thấy nhiều confident error là *false positive* — spoof bị cho score bonafide rất cao — đặc biệt ở các attack A26/A17/A28; đây là loại lỗi nguy hiểm trong deployment vì audio giả bị chấp nhận như thật. Phần này được dùng để chọn mẫu nghe/soi spectrogram ở Thực tập 2, không rút kết luận định lượng theo attack hay codec.
...
![Hình 3.3. Tương quan score giữa các mô hình trên ASVspoof 5 Track 1 eval. Hình hiển thị cả AASIST3 do là artifact thô đã chạy trước, nhưng các con số liên quan tới AASIST3 không được dùng trong diễn giải vì checkpoint Hugging Face công khai không trùng weights paper (xem §3.1).](../results/error_analysis/asvspoof5/plots/score_correlation.png)
