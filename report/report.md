...
ASVspoof 5 chỉ ra hai failure mode chính: codec dominance (EER nhóm XLS-R tăng từ
...
Phần còn lại của báo cáo được tổ chức như sau. **Chương 2** trình bày cơ sở lý thuyết, bao gồm phát biểu bài toán, các loại tấn công, các tập dữ liệu benchmark, các nhóm kiến trúc tiếp cận, và phương pháp đánh giá. **Chương 3** mô tả thiết lập thí nghiệm, trình bày kết quả EER tổng hợp trên năm mô hình trong benchmark chính và bốn dataset, phân tích lỗi diagnostic trên ASVspoof 5, đồng thời giải thích trường hợp checkpoint AASIST3 bị loại khỏi phân tích chính. **Chương 4** tổng kết pilot benchmark, đề xuất hướng xây bản đồ dữ liệu/kiến trúc cho SDD, và nêu các hướng ứng viên ở mức ý niệm cho Thực tập 2 và Luận văn.
...
| ASVspoof 2021 DF | 2021 | LA/deepfake + codec condition | Anh | 458,868 utterances trong eval subset dùng ở project | Audio kế thừa ASV2019, re-encode qua nhiều codec/bitrate | Reproduce: stress test cho codec robustness |
...
| In-the-Wild | 2022 | In-the-wild deployment condition | Chủ yếu Anh | 31,779 utterances, 58 speakers trong bản dùng ở project | Scraped từ nguồn công khai, 20.8h bonafide và 17.2h spoofed audio | Reproduce: probe cho cross-domain generalization [4] |
...
| WaveFake | 2021 | TTS/vocoder | Anh + Nhật | Khoảng 104K utterances theo paper | Tập trung GAN/neural vocoder artifacts | Tham chiếu lịch sử cho vocoder artifacts; không đủ hiện đại làm main benchmark [33] |
...
Tóm lại, ASVspoof 5 eval xác nhận ba điểm chính: (i) tất cả mô hình đều có generalization gap lớn so với ASV19 LA; (ii) với nhóm XLS-R, EER cao trên ASV5 chủ yếu đến từ một số codec khó như C04/C07, nhưng ở mức attack ID vẫn tồn tại confident FP đáng chú ý trên A28; (iii) failure overlap và score correlation không hoàn toàn trùng nhau giữa các họ mô hình. Các quan sát này tạo động lực cho những hướng ứng viên ở §4.4, nhưng chưa đủ để chốt một đề xuất cải thiện cụ thể.
...
- **ASVspoof 5 cho thấy failure mode theo codec/encoding.** Với hai mô hình XLS-R, subset `nocodec` có EER thấp hơn nhiều so với C04/C07, gợi ý rằng codec robustness là một trục đánh giá cần được tách riêng.
...
- **Cross-domain và multilingual adaptation.** Nếu mở rộng sang MLAAD/SpeechFake/In-the-Wild-like data, cần kiểm tra liệu detector học cue phụ thuộc tiếng Anh hoặc dataset source hay học được cue spoof tổng quát.
...
- **Phân tích định tính lỗi.** Các confident errors như trong `hard_errors.csv` nên được nghe và soi spectrogram để hiểu cue âm học nào bị bỏ sót; phần này bổ sung cho EER và grouped metrics.
