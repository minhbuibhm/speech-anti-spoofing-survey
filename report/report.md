...
Bên cạnh phần khảo sát, báo cáo tái lập benchmark chính với năm mô hình tiêu biểu gồm
LFCC+LCNN, AASIST, AASIST-L, XLS-R+AASIST và XLS-R+Nes2Net trên bốn tập dữ liệu:
ASVspoof 2019 LA, ASVspoof 2021 DF, ASVspoof 5 Track 1 và In-the-Wild. AASIST3 được
khảo sát ở mức checkpoint công khai nhưng không đưa vào phân tích chính vì model card
Hugging Face cho biết checkpoint hiện tại đã deprecated và không phải weights dùng trong
...
và generalization gap nhỏ hơn trên hầu hết các dataset được khảo sát. Phân tích lỗi trên
ASVspoof 5 chỉ ra hai dạng lỗi chính: codec dominance (EER nhóm XLS-R tăng từ
2.71-3.78% trên nocodec lên 35-42% trên C04/C07) và confident false positive theo attack ID
(A28 đứng đầu FP rate cho cả hai mô hình XLS-R, A17 đứng đầu cho AASIST), tạo động lực
cho hai hướng ứng viên cụ thể là codec-aware training và attack-aware hard mining. Tuy nhiên,
phạm vi hiện tại vẫn là một pilot
...
Một CM system điển hình gồm ba bước. **Front-end** chuyển waveform thành representation: có thể là hand-crafted spectral features như LFCC/CQCC, feature học trực tiếp từ raw waveform, hoặc representation từ self-supervised learning (SSL) model như Wav2Vec2, XLS-R, WavLM. **Back-end** nhận representation này để mô hình hoá quan hệ thời gian, phổ hoặc dấu hiệu ở mức speaker/utterance. **Classifier/scoring head** sinh score cuối cùng cho từng utterance. Cách tách `front-end` và `back-end` này giúp so sánh các nhóm kiến trúc ở §2.3: khác biệt chính giữa LFCC+LCNN, AASIST và XLS-R+Nes2Net không chỉ nằm ở classifier, mà nằm ở loại representation mà mô hình được phép khai thác.

Về threat model, báo cáo này gom các kịch bản SDD thành bốn nhóm chính theo cả cơ chế tấn công và điều kiện triển khai. Các nhóm này không hoàn toàn trực giao, vì access scenario, deployment condition, generation family và adversarial threat là các trục phân loại khác nhau, nhưng cách gom này đủ để đặt phạm vi thực nghiệm của báo cáo:

- **Logical Access (LA).** Attacker tiêm audio giả trực tiếp vào pipeline số, không cần phát lại qua loa/micro. Hai dạng phổ biến là **Text-to-Speech (TTS)** và **Voice Conversion (VC)**. TTS sinh giọng nói từ văn bản; VC giữ nội dung hoặc đặc tính ngữ âm của nguồn nhưng biến đổi speaker identity sang giọng đích. LA là kịch bản chính của ASVspoof 2019 LA, ASVspoof 2021 DF và ASVspoof 5 Track 1 [1], [2]. Các dataset gần đây như MLAAD và SpeechFake cho thấy quy mô và độ đa dạng của TTS/VC hiện đại đã mở rộng mạnh theo ngôn ngữ, công cụ sinh và phương pháp generation [8], [29].
- **Physical Access (PA) / replay.** Attacker phát lại audio qua thiết bị vật lý rồi thu hoặc đưa vào hệ thống qua microphone. Khác với LA, replay chứa thêm dấu vết của loa, phòng, microphone và khoảng cách thu. Do đó một detector huấn luyện cho LA không nhất thiết generalize sang PA, và ngược lại. Các benchmark gần đây như EchoFake còn kết hợp zero-shot TTS với physical replay để mô phỏng kịch bản triển khai thực tế hơn [1], [23].
- **Deepfake/in-the-wild deployment condition.** Audio được thu thập từ social media, video streaming hoặc nguồn công khai, thường đã qua codec, noise, trimming, post-processing và có metadata không đầy đủ. Đây không phải một attack mechanism đơn lẻ, mà là một deployment-like condition dùng để kiểm tra cross-domain generalization [4], [15].
- **Adversarial và neural codec attacks.** Adversarial attacks thêm perturbation có chủ đích để làm sai lệch detector, trong khi neural codec attacks liên quan đến các TTS/VC pipeline hiện đại sử dụng codec representation như EnCodec/SNAC/Vocos, khiến artifact không còn giống các spectral artifact truyền thống trong benchmark cũ [21], [22]. ASVspoof 5 đưa adversarial/deepfake setting và neural encoding/compression vào benchmark ở quy mô lớn hơn các phiên bản trước [3], [16], [17].

Ngoài bốn nhóm trên, partial spoofing hoặc localized manipulation là một mở rộng quan trọng: chỉ một đoạn trong utterance bị chỉnh sửa, nên bài toán cần thêm segment-level label hoặc localization metric thay vì chỉ utterance-level EER [20], [18], [19]. Báo cáo này tập trung thực nghiệm vào nhóm LA và deepfake/in-the-wild vì bốn dataset reproduce ở Chương 3 chủ yếu thuộc hai nhóm này. PA/replay, adversarial robustness và partial/localized spoofing được trình bày ở mức cơ sở lý thuyết để làm rõ bức tranh nghiên cứu, nhưng chưa phải trọng tâm thực nghiệm của Thực tập 1.

### 2.2 Các tập dữ liệu benchmark

Dataset trong SDD không chỉ khác nhau về số lượng sample, mà còn khác nhau về giả định đánh giá. Có thể mô tả dataset theo bốn trục chính: **attack/access scenario** (Logical Access, Physical Access/replay, partial manipulation), **recording/deployment condition** (clean lab, codec-compressed, in-the-wild, replay-aware), **generation family** (vocoder-era TTS/VC, neural codec/CoSG, diffusion/zero-shot TTS, adversarial) và **language coverage** (English-centric, Mandarin-heavy, multi-lingual). Bốn dataset được reproduce trong báo cáo không nhằm bao trùm toàn bộ landscape, mà đại diện cho bốn dạng kiểm tra chính trong scope Thực tập 1: clean benchmark, codec robustness, modern ASVspoof attacks và in-the-wild domain shift.
...
| Dataset | Năm | Kịch bản/điều kiện | Ngôn ngữ | Kích thước dùng/tham chiếu | Đặc điểm chính | Vai trò/lý do trong báo cáo |
|---------|-----|----------|----------|----------------------------|----------------|-------------------------|
...
| ASVspoof 2021 DF | 2021 | LA/deepfake + codec condition | Anh | 458,868 utterances trong eval subset dùng ở project | Audio kế thừa ASV2019, re-encode qua nhiều codec/bitrate | Reproduce: kiểm tra codec robustness |
| ASVspoof 5 Track 1 | 2024/2025 | LA/deepfake + modern/adversarial context | Anh (MLS English partition) | 680,774 utterances trong eval split dùng ở project | Crowdsourced speech; toàn database có 32 attack algorithms, Track 1 eval có 16 attacks; có metadata codec/encoding | Reproduce: trọng tâm error analysis sơ bộ trên modern benchmark [16], [17] |
| In-the-Wild | 2022 | In-the-wild deployment condition | Chủ yếu Anh | 31,779 utterances, 58 speakers trong bản dùng ở project | Scraped từ nguồn công khai, 20.8h bonafide và 17.2h spoofed audio | Reproduce: kiểm tra cross-domain generalization [4] |
...
| CodecFake / CodecFake+ | 2024/2025 | Neural codec / CoSG generation | Nhiều nguồn | CodecFake+ dùng 31 codec models cho train và 17 CoSG models cho eval | Nhắm trực tiếp vào neural codec / CoSG generation | Không reproduce: rất phù hợp để follow-up neural codec robustness, nhưng mới và lớn; ASVspoof 5 đang là benchmark hiện đại chính [21], [22] |
...
| ADD 2022/2023 | 2022/2023 | Partial/localized + low-quality/fake-game tasks | Mandarin-heavy | Nhiều track challenge | Mở rộng ngoài binary utterance-level detection | Không reproduce: task/protocol khác với pipeline EER utterance-level [18], [19] |
...
| WaveFake | 2021 | TTS/vocoder | Anh + Nhật | Khoảng 104K utterances theo paper | Tập trung GAN/neural vocoder artifacts | Tham chiếu lịch sử cho vocoder artifacts; không đủ hiện đại làm benchmark chính [33] |
| FoR | 2019 | TTS | Anh | Khoảng 198K utterances theo paper | Dataset real/fake speech đời đầu, nhiều TTS cũ | Tham chiếu lịch sử, không dùng làm benchmark chính [34] |
...
**SSL/foundation front-end + anti-spoofing back-end.** Nhóm này dùng Wav2Vec2, XLS-R, WavLM, HuBERT hoặc speech encoder lớn làm front-end, sau đó gắn back-end như AASIST, MFA, Nes2Net, pooling/MLP, state-space model hoặc layer-selection module [7], [9], [10], [35]. Lợi thế kỳ vọng là SSL representation chứa prior về phonetic structure, speaker/channel variation và acoustic regularity từ pre-training quy mô lớn, nên có nền tảng ổn định hơn khi attack hoặc codec thay đổi. Các paper tiêu biểu gồm Wav2Vec2/XLS-R + AASIST của Tak et al. [12], WavLM + Multi-Fusion Attentive classifier của Guo et al. [24], AASIST3 [11], WavLM back-ends/fusion trong ASVspoof 5 [25], [26], Nes2Net [13], và XLSR-Mamba [30]. Hạn chế chính là chi phí: riêng XLS-R 300M hoặc WavLM Large đã lớn hơn nhiều so với AASIST/LFCC+LCNN, và hiệu năng phụ thuộc checkpoint, layer selection, fine-tuning, augmentation và calibration.
...
| Nes2Net follow-up/preprint | Nes2Net-LA / MultiAPI Spoof [31] | Local-attention extension của Nes2Net và dataset multi-API | Chưa reproduce; mới, cần đọc thận trọng khi kết luận SOTA |
| Reproduced SSL systems | XLS-R+AASIST, Nes2Net [12], [13] | Liên hệ trực tiếp với benchmark chính ở Chương 3 | Đã đưa vào bảng EER |
...
Bảng 2.3 tổng hợp các mô hình được reproduce hoặc khảo sát trong quá trình chuẩn bị Chương 3. Các giá trị hiệu năng trong cột "Kết quả paper" chỉ để đặt mô hình vào bối cảnh, không dùng thay cho kết quả reproduce. Các metric khác loại như EER, min t-DCF và minDCF không được so sánh trực tiếp với nhau.

**Bảng 2.3.** So sánh các mô hình được tái lập hoặc khảo sát.
...
| AASIST3 | Wav2Vec2 SSL features | KAN bridge + modified AASIST | Hàng trăm triệu params nếu tính SSL | Kiến trúc literature; checkpoint public bị loại khỏi benchmark chính ở §3.1 | Metric paper là minDCF trong challenge condition; checkpoint HF public deprecated và không trùng paper weights | ASVspoof 2024: minDCF 0.5357 closed, 0.1414 open [11], [32] |
...
Chương 2 đặt nền tảng lý thuyết và literature cho phần thực nghiệm bằng năm khối nội dung. §2.1 định nghĩa Speech Deepfake Detection ở mức utterance-level (score liên tục $s = f(x)$ so với threshold $\tau$, quy ước *bonafide* = 1, *spoof* = 0), tách kiến trúc CM thành front-end / back-end / scoring head, và phân nhóm threat/deployment setting thành Logical Access (TTS/VC), Physical Access/replay, deepfake/in-the-wild và adversarial/neural codec attacks; báo cáo tập trung thực nghiệm vào LA và deepfake/in-the-wild, còn PA/replay, adversarial robustness và partial/localized spoofing được giữ ở mức bối cảnh. §2.2 tổng hợp các benchmark theo bốn trục (attack/access scenario, recording/deployment condition, generation family, language coverage) và xác định bốn dataset reproduce — ASVspoof 2019 LA (clean), ASVspoof 2021 DF (codec), ASVspoof 5 Track 1 (modern attacks), In-the-Wild (domain shift) — đồng thời nêu các dataset không reproduce nhưng quan trọng cho literature (MLAAD v9, SpeechFake, CodecFake/CodecFake+, EchoFake, ADD, PartialSpoof). §2.3 phân loại bốn nhóm kiến trúc (signal/task-specific supervised, raw waveform end-to-end, SSL front-end + anti-spoofing back-end, system-level robustness), đặt các mô hình reproduce vào bối cảnh đó, và tách riêng AASIST3 như một kiến trúc literature có checkpoint public không đủ điều kiện so sánh chính ở Chương 3; báo cáo cẩn thận không gọi Nes2Net-X là SOTA tuyệt đối mà là một đại diện mạnh của lớp SSL + lightweight back-end giai đoạn 2024-2025. §2.4 trình bày bộ metric đánh giá với EER là metric chính, cùng minDCF/min t-DCF/a-DCF cho deployment và SASV, ROC/DET/AUC cho trực quan trade-off, calibration metrics (log loss, Brier, ECE, $C_{\mathrm{llr}}$, actual DCF) cho các phân tích bổ sung, và metric mở rộng cho partial-spoof/localization. §2.5 tổng hợp bốn thách thức xuyên suốt — cross-domain generalization, codec robustness, modern attacks/adversarial setting, multi-linguality và deployment constraints — và ánh xạ trực tiếp mỗi thách thức tới một dataset reproduce, từ đó tạo khung diễn giải cho các phân tích ở Chương 3.

---

## Chương 3: Reproduce và phân tích kết quả

Chương này tái lập benchmark chính với năm mô hình anti-spoofing trên bốn dataset đã giới thiệu ở Chương 2, sau đó tiến hành phân tích lỗi trên ASVspoof 5 và phân tích cross-dataset để rút ra các phát hiện. Báo cáo phân biệt rõ hai thành phần: (i) các checkpoint *pretrained* đủ điều kiện so sánh trong benchmark chính và (ii) phần *do project thực hiện* — bao gồm pipeline đánh giá thống nhất, cấu hình eval, error analysis và phân tích so sánh.
...
2. **Cross-dataset benchmark năm mô hình × bốn dataset** (§3.2–§3.3, §3.5). Trong số năm mô hình có đại diện hand-crafted (LFCC+LCNN), end-to-end nhỏ (AASIST/AASIST-L) và hai representative SSL systems dùng XLS-R 300M (XLS-R+AASIST, XLS-R+Nes2Net). Bốn dataset bao quát clean benchmark (ASV19 LA), codec-stressed (ASV21 DF), modern-attack (ASV5 Track 1) và in-the-wild domain shift (ITW).
3. **Phân tích lỗi trên ASV5** (§3.4) theo attack, attack tag, codec, score distribution, score correlation và failure overlap — giúp tách *codec robustness gap* ra khỏi *modern-attack robustness gap* và làm rõ điều kiện mà SSL front-end thực sự có lợi.
...
**Mã nguồn và cấu trúc.** Mỗi dataset được đánh giá bằng một notebook riêng ([notebooks/eval_asvspoof_2019.ipynb](notebooks/eval_asvspoof_2019.ipynb), [notebooks/eval_asvspoof_2021.ipynb](notebooks/eval_asvspoof_2021.ipynb), [notebooks/eval_asvspoof_5.ipynb](notebooks/eval_asvspoof_5.ipynb), [notebooks/eval_in_the_wild.ipynb](notebooks/eval_in_the_wild.ipynb)), được sinh tự động từ template chung trong [scripts/create_eval_notebooks.py](scripts/create_eval_notebooks.py). Cách tổ chức này giữ logic suy luận nhất quán giữa các dataset (cùng cách load checkpoint, cùng cách tính EER) trong khi vẫn cho phép tuỳ biến phần parser metadata cho từng dataset. Việc bổ sung mô hình hoặc dataset mới ở Thực tập 2 chỉ cần thêm một hàm load/parser tương ứng và sinh lại notebook. Để hỗ trợ khả lập và đối chiếu kết quả, toàn bộ mã nguồn của project, bao gồm các Python notebooks, script sinh notebooks, các file kết quả, bản report và mã nguồn LaTeX, được lưu tại repository GitHub: https://github.com/minhbuibhm/internship-1-sdd.

**Nguồn checkpoint.** Bốn mô hình được dùng ở chế độ pretrained, không fine-tune thêm trong phạm vi báo cáo: AASIST và AASIST-L từ repository chính thức của tác giả [AASIST GitHub](https://github.com/clovaai/aasist); XLS-R 300M từ release Fairseq [XLS-R 300M checkpoint](https://dl.fbaipublicfiles.com/fairseq/wav2vec/xlsr2_300m.pt); XLS-R + AASIST từ checkpoint công bố bởi nhóm SSL-AASIST (Tak et al., 2022); XLS-R + Nes2Net từ release của nhóm Nes2Net (Liu et al., 2025). Riêng LFCC+LCNN được huấn luyện lại từ đầu trên ASVspoof 2019 LA (~10 epoch) do không có checkpoint công khai phù hợp; vì vậy kết quả của LFCC+LCNN nên được hiểu là *project-specific baseline* chứ không đại diện cho best published LFCC+LCNN recipe.

**Checkpoint bị loại khỏi benchmark chính.** AASIST3 vẫn được trình bày ở Chương 2 như một kiến trúc trong literature, nhưng kết quả suy luận từ checkpoint Hugging Face công khai không được đưa vào bảng EER, ranking, error analysis hay kết luận của Chương 3. Trong notebook, checkpoint được load bằng identifier `MTUCI/AASIST3`; tại thời điểm viết báo cáo, identifier này redirect tới `lab260/AASIST3` trên Hugging Face. Model card của `lab260/AASIST3` ghi rõ model đã deprecated, khuyến nghị dùng phiên bản cập nhật, và weights public trong repository không phải weights dùng cho kết quả paper [32]. Vì vậy EER đã chạy được xem là artifact kiểm tra reproducibility/checkpoint, không phải bằng chứng về năng lực của kiến trúc AASIST3 hay kết quả paper [11].

**Cấu trúc kết quả.** Toàn bộ điểm số đầu ra được lưu vào các file `results/<dataset>/results.pkl`. Mỗi file là một `dict` Python với các khoá là tên mô hình; các pkl mới (ASV5, In-the-Wild) còn có thêm một khoá tổng `__metadata__` cho thông tin chung (tên dataset, split, số utterance, mode tính EER, thời điểm sinh):
...
- **Dải EER trải rộng đáng kể** giữa các mô hình trên cùng một dataset. Trên ASVspoof 2019 LA, khoảng cách giữa mô hình tốt nhất quan sát được (XLS-R + Nes2Net, 0.45%) và kém nhất trong benchmark chính (LFCC+LCNN, 19.64%) là khoảng 19 điểm phần trăm. Trên In-the-Wild, khoảng cách này rộng hơn nữa (5.57% so với 70.23%).
...
Các nhận xét trên chỉ giới hạn trong phạm vi năm mô hình của benchmark chính và bốn dataset đã khảo sát. Phần §3.4 sẽ bóc tách EER cao trên ASV5 thành các thành phần (codec, attack family) để chỉ ra điều gì đang làm các mô hình thất bại.
...
![Hình 3.1. Phân phối score của các mô hình trên ASVspoof 5 Track 1 eval. Đường AASIST3 vẫn xuất hiện trên hình do là artifact thô đã chạy trước, nhưng không được dùng trong diễn giải vì checkpoint Hugging Face công khai không trùng weights paper (xem §3.1).](../results/error_analysis/asvspoof5/plots/score_distributions.png)
...
**3.4.3 Lỗi theo codec.** Theo mô tả dữ liệu ASVspoof 5, điều kiện C00 là không mã hoá/nén và được ghi là `nocodec` trong artifact phân tích của báo cáo. Các điều kiện C01-C07 giữ băng thông 16 kHz, trong đó C04 dùng Encodec và C07 dùng chuỗi nén kép MP3+Encodec; các điều kiện C08-C11 là narrow-band 8 kHz, gồm Opus, AMR, Speex và một điều kiện varied mô phỏng các pipeline gọi điện/PSTN [16].

| Mã | Codec / xử lý | Băng thông / bitrate | Vai trò trong phân tích |
|----|---------------|----------------------|--------------------------|
| C00 / `nocodec` | Không encoding/compression | 16 kHz, không bitrate | Điều kiện tham chiếu; EER thấp của nhóm XLS-R trên subset này cho thấy bản thân attack không phải nguồn lỗi duy nhất. |
...
ASVspoof 5 còn có C01 Opus 16 kHz, C02 AMR 16 kHz, C03 Speex 16 kHz, C08 Opus 8 kHz, C09 AMR 8 kHz và C10 Speex 8 kHz [16]. Codec là phát hiện nổi bật nhất trong error analysis ASV5, đặc biệt với nhóm XLS-R. Với hai mô hình XLS-R, subset `nocodec` có EER rất thấp so với EER tổng: XLS-R + Nes2Net đạt 2.71% và XLS-R + AASIST đạt 3.78%. Một số codec khác cũng tương đối thấp, như C05 (4.11% / 4.81%), C11 (5.82% / 8.01%) và C06 (8.20% / 7.77%). Ngược lại, C07 và C04 làm EER tăng rất mạnh: XLS-R + Nes2Net đạt 42.32% ở C07 và 41.17% ở C04; XLS-R + AASIST đạt 36.87% ở C07 và 35.79% ở C04. Điều này cho thấy EER cao của nhóm XLS-R trên ASV5 chủ yếu phản ánh codec robustness gap, không chỉ modern attack robustness nói chung. LFCC+LCNN cũng có EER rất cao ở C05 (51.55%), C07 (50.96%) và C02 (48.01%); ngay cả subset `nocodec` cũng ở mức 30.62%, tức không có vùng điều kiện dễ tương tự nhóm XLS-R. Sự tương phản này cho thấy LFCC+LCNN bị giới hạn bởi feature design và domain mismatch rộng hơn, không chỉ bởi một vài codec khó. Kết quả theo codec là một tín hiệu để đưa codec-aware training vào nhóm hướng ứng viên ở Chương 4, nhưng cần được kiểm chứng trên bản đồ dữ liệu rộng hơn. Phần §3.4.4 bổ sung kiểm tra định tính trên cả false positive theo attack ID và false negative theo C04/C07 để tách security gap khỏi usability/calibration gap.

![Hình 3.2. EER theo codec trên ASVspoof 5 Track 1 eval. Hình hiển thị cả AASIST3 do là artifact thô đã chạy trước, nhưng không được dùng trong diễn giải vì checkpoint Hugging Face công khai không trùng weights paper (xem §3.1); câu chuyện codec gap C04/C07 vẫn được rút ra từ năm mô hình trong benchmark chính.](../results/error_analysis/asvspoof5/plots/grouped_eer_codec.png)

**3.4.4 Confident errors.** Để kiểm tra định tính các lỗi tự tin cao, báo cáo trực quan hóa một số audio bằng waveform, LTAS, STFT spectrogram, log-mel spectrogram, LFCC-style features và các descriptor phổ. Phần này tách hai loại lỗi có ý nghĩa khác nhau: false positive trên spoof attack là rủi ro security vì mẫu giả bị chấp nhận là bonafide, còn false negative trên bonafide qua codec khó là usability/calibration gap. Các mẫu dưới đây đều là minh hoạ định tính; kết luận định lượng vẫn dựa trên grouped EER và FP rate theo attack.

Trên nhóm false positive, hai attack ID nổi bật từ kết quả phân tích lỗi là A28 và A17. A28 là attack có FP rate cao nhất cho cả hai mô hình XLS-R: `XLS-R+AASIST` 0.17% và `XLS-R+Nes2Net` 3.01%. A17 là attack có FP rate cao nhất cho AASIST (11.96%), đứng thứ hai cho AASIST-L (8.98%) và cũng nằm trong top-2 của hai mô hình XLS-R. Vì vậy, hai mẫu `nocodec` được chọn để loại bỏ nhiễu codec và tập trung vào tương tác giữa attack quality và representation của mô hình.

![Hình 3.3. Audio panel của mẫu `E_0004782150` — spoof A28 `nocodec` bị `XLS-R+Nes2Net` phân loại nhầm thành bonafide với score khoảng 0.99999.](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0004782150.png)

Ở mẫu A28 này, LTAS của hard error và bonafide reference khá gần nhau ở dải 0-7 kHz; STFT/mel vẫn cho thấy harmonic structure và formant trajectory tương đối tự nhiên. Tuy nhiên, hard error có high-frequency roll-off rõ ở khoảng 7.5-8 kHz, với mức năng lượng thấp hơn bonafide reference khoảng 35-40 dB. Đây là một dấu hiệu spoof tiềm năng nhưng nằm trong band hẹp, và `XLS-R+Nes2Net` vẫn gán score bonafide gần 1 trong điều kiện `nocodec`. Quan sát này gợi ý A28 có thể tiến gần vùng representation bonafide ở các dấu hiệu chính mà mô hình khai thác, dù vẫn còn dấu hiệu phổ có thể nhận ra bằng mắt.

![Hình 3.4. Audio panel của mẫu `E_0002841318` — spoof A17 `nocodec` bị AASIST phân loại nhầm thành bonafide với score khoảng 0.999998.](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0002841318.png)

Mẫu A17 cho thấy kết quả ngược lại: dấu hiệu phổ nhìn thấy rõ hơn nhưng AASIST vẫn dự đoán bonafide rất tự tin. LTAS của hard error cao hơn bonafide reference ở phần lớn dải mid-high, đặc biệt khoảng 1-6 kHz, cho thấy spectral tilt nông hơn natural speech; STFT/mel có các dải năng lượng dọc khá đều, gợi ý artifact synthesis hoặc vocoder. Tuy nhiên các dấu hiệu này không được AASIST khai thác đủ ở ngưỡng quyết định hiện tại. Cặp A28/A17 vì vậy minh hoạ hai dạng lỗi khác nhau: một attack ít để lại dấu hiệu phổ rõ cho SSL model, và một attack có dấu hiệu phổ nhưng raw-waveform model không nhận ra.

False negative trên C04/C07 vẫn quan trọng nhưng mang ý nghĩa khác: đây là bonafide bị reject, nên phản ánh usability/calibration gap hơn là security gap. Hai mẫu tiêu biểu là `E_0002108564` (bonafide, C04) và `E_0001333311` (bonafide, C07). Cả hai đều là false negative của `XLS-R+Nes2Net`: score bonafide rất thấp (lần lượt khoảng \(3.0\times 10^{-7}\) và \(1.3\times 10^{-5}\)), tương ứng với confidence sai gần 1. Trong các panel trực quan, đường LTAS của hard error và bonafide reference cùng codec khá gần nhau; STFT/mel vẫn thể hiện harmonic structure và formant tương đối tự nhiên; không có dấu hiệu spoof rõ ràng bằng mắt ở phổ vĩ mô. Quan sát này củng cố nhận định rằng lỗi C04/C07 nhiều khả năng liên quan tới codec footprint của Encodec hoặc MP3+Encodec làm lệch representation hoặc calibration của mô hình, dùng để hỗ trợ F1/F2 chứ không thay thế grouped EER.

![Hình 3.5. Audio panel của mẫu `E_0002108564` — bonafide C04 bị `XLS-R+Nes2Net` phân loại nhầm thành spoof với score bonafide khoảng \(3.0\times 10^{-7}\).](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0002108564.png)

Ở mẫu C04 này, waveform và spectrogram cho thấy speech liên tục với harmonic structure rõ. Đường LTAS của hard error và bonafide reference cùng codec nhìn chung gần nhau, nhưng hard error có mid-band excess nhỏ và nhất quán khoảng 3-6 kHz. Không có dấu hiệu spoof rõ kiểu attack, nhưng model vẫn gán score bonafide gần 0. Điều này gợi ý rằng Encodec có thể tạo footprint phổ làm representation của bonafide lệch khỏi vùng mà `XLS-R+Nes2Net` xem là bonafide.

![Hình 3.6. Audio panel của mẫu `E_0001333311` — bonafide C07 bị `XLS-R+Nes2Net` phân loại nhầm thành spoof với score bonafide khoảng \(1.3\times 10^{-5}\).](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0001333311.png)

Mẫu C07 cho thấy dạng lỗi tương tự nhưng dưới chuỗi nén kép MP3+Encodec. LTAS của hard error và bonafide reference cùng codec gần như trùng nhau, cùng có roll-off ở vùng tần số cao; STFT và mel vẫn giữ cấu trúc harmonic/formant tự nhiên. Việc model vẫn rất tự tin reject bonafide cho thấy vấn đề không nằm ở attack family, mà nhiều khả năng ở codec footprint của C07 và calibration/representation của mô hình khi gặp codec này.

**3.4.5 Failure overlap và score correlation giữa các mô hình.** Ma trận overlap cho thấy AASIST và AASIST-L có overlap lỗi cao nhất (0.62), phù hợp vì hai mô hình cùng họ kiến trúc. Hai mô hình XLS-R có overlap 0.31, thấp hơn cặp AASIST/AASIST-L, cho thấy back-end khác nhau vẫn tạo ra một phần lỗi riêng. Score correlation củng cố nhận xét này: AASIST và AASIST-L tương quan cao (0.84), hai mô hình XLS-R tương quan vừa phải (0.49), trong khi một số cặp khác họ thấp hơn, ví dụ AASIST với LFCC+LCNN (-0.08) và LFCC+LCNN với XLS-R+AASIST (0.13). Điều này hỗ trợ hướng score-level fusion, nhưng fusion cần được kiểm chứng bằng thí nghiệm riêng vì overlap/correlation chỉ là tín hiệu gián tiếp.

![Hình 3.7. Tương quan score giữa các mô hình trên ASVspoof 5 Track 1 eval. Hình hiển thị cả AASIST3 do là artifact thô đã chạy trước, nhưng các con số liên quan tới AASIST3 không được dùng trong diễn giải vì checkpoint Hugging Face công khai không trùng weights paper (xem §3.1).](../results/error_analysis/asvspoof5/plots/score_correlation.png)

Tóm lại, ASVspoof 5 eval xác nhận ba điểm chính: (i) tất cả mô hình đều có generalization gap lớn so với ASV19 LA; (ii) với nhóm XLS-R, EER cao trên ASV5 chủ yếu đến từ một số codec khó như C04/C07, nhưng ở mức attack ID vẫn tồn tại confident FP đáng chú ý trên A28; (iii) failure overlap và score correlation không hoàn toàn trùng nhau giữa các họ mô hình. Các quan sát này tạo động lực cho những hướng ứng viên ở §4.4, nhưng chưa đủ để chọn ra một đề xuất cải thiện cụ thể.
...
- **F2 — Dịch chuyển phân phối codec giữa ASV21 DF và ASV5.** Hai mô hình XLS-R có generalization gap chỉ khoảng +2.5 pp trên ASV21 DF nhưng tăng lên +18.43 đến +21.13 pp trên ASV5. Điều này gợi ý codec/encoding condition khó trong ASV5 không trùng phân bố với codec truyền thống của ASV21 DF — ASV21 DF không đủ để đo codec robustness thực tế. *(§3.4.3 + §3.5 → §4.2: cần mở rộng nhóm dataset codec/transmission/neural codec.)*
- **F3 — Attack tag thô không giải thích hết lỗi, nhưng attack ID vẫn quan trọng.** EER theo attack tag (AC1/AC2/AC3) gần đồng nhất ở mọi mô hình (AASIST 35.29-36.05%, LFCC+LCNN 42.05-42.83%, XLS-R + AASIST 19.55-19.72%, XLS-R + Nes2Net 21.56-21.61%), nên bottleneck tổng thể của nhóm XLS-R trên ASV5 vẫn nằm nhiều ở codec/encoding hơn là tag attack thô. Tuy nhiên, ở mức attack ID, confident FP tập trung rõ vào A28 cho hai mô hình XLS-R (đứng đầu FP rate) và A17/A26 cho họ AASIST; A17 còn nằm top-2 FP của cả hai mô hình XLS-R. *(§3.4.2, §3.4.4 → §4.2, §4.4: benchmark map cần tách attack family khỏi codec/channel condition, đồng thời vẫn cần attack-aware hard mining.)*
- **F4 — Failure overlap và score correlation gợi ý cơ hội fusion.** Cặp AASIST/AASIST-L overlap 0.62 và correlation 0.84 (cùng họ), trong khi một số cặp khác họ thấp hơn (AASIST với LFCC+LCNN −0.08, LFCC+LCNN với XLS-R+AASIST 0.13). Hai mô hình XLS-R có correlation 0.49 — vẫn còn lỗi riêng dù cùng front-end family. Đây là tín hiệu gián tiếp cho score-level fusion. *(§3.4.5 → §4.4: fusion/calibration là candidate direction, cần thí nghiệm riêng.)*
...
- **LFCC+LCNN là project-specific baseline** được huấn luyện lại từ đầu (~10 epoch) và không đại diện cho cấu hình tốt nhất được công bố; các phát biểu liên quan tới LFCC+LCNN trong báo cáo cần được hiểu trong phạm vi này.
...
Các phát hiện và giới hạn ở §3.6 định hình roadmap ở Chương 4: mở rộng bản đồ dữ liệu/kiến trúc, chuẩn hoá protocol đánh giá, lựa chọn baseline sau khi map đủ rộng hơn, và xem các hướng như codec-aware training, attack-aware hard mining, fusion/calibration hoặc front-end/back-end ablation như các hướng ứng viên cần kiểm chứng.

### 3.7 Tóm tắt chương

Chương 3 chuyển từ literature sang thực nghiệm với ba đóng góp đã nêu ở đầu chương: pipeline đánh giá thống nhất, cross-dataset benchmark năm mô hình × bốn dataset, và phân tích lỗi trên ASVspoof 5. §3.1 mô tả thiết lập trên Kaggle T4, sinh notebook eval từ template chung và lưu kết quả theo định dạng `results.pkl` thống nhất (khoá `__metadata__` cho các pkl mới); phân biệt rõ bốn checkpoint pretrained đủ điều kiện so sánh, LFCC+LCNN project-specific baseline (huấn luyện lại ~10 epoch), và checkpoint AASIST3 public bị loại khỏi phân tích chính. §3.2 trình bày bảng EER tổng hợp (Bảng 3.1) cùng bốn quan sát: dải EER trải rộng giữa các mô hình, *leaderboard inversion* giữa các dataset, khoảng cách giữa nhóm XLS-R 300M và phần còn lại nới rộng theo độ khó, và modern-attack robustness vẫn chưa được giải quyết. §3.3 phân tích EER theo từng dataset, làm rõ vai trò của ASVspoof 2019 LA (clean), ASVspoof 2021 DF (codec stress), ASVspoof 5 (modern attacks) và In-the-Wild (domain shift), kèm các lưu ý cho từng trường hợp (LFCC+LCNN project-baseline, label noise của ITW, loại checkpoint của  AASIST3). §3.4 — phần phân tích lỗi sâu chính của báo cáo — bóc tách EER cao trên ASV5 thành các thành phần: phân phối điểm số, EER theo attack ID và attack tag, EER theo codec, confident errors (qualitative), và failure overlap + score correlation; phát hiện nổi bật là EER cao của nhóm XLS-R chủ yếu đến từ C04/C07 (`nocodec` chỉ 2.71-3.78%), trong khi attack tag AC1/AC2/AC3 gần đồng nhất ở các mô hình chính nhưng attack ID cụ thể vẫn tạo confident FP rõ trên A28/A17. Kiểm tra định tính trên audio hard-error cho thấy A28 `nocodec` có thể qua `XLS-R+Nes2Net` dù không để lại dấu vết phổ vĩ mô rõ, A17 `nocodec` có dấu hiệu phổ thấy được nhưng AASIST vẫn dự đoán bonafide, còn hai mẫu bonafide C04/C07 bị `XLS-R+Nes2Net` gán bonafide score gần 0 củng cố giả thuyết codec footprint làm lệch representation/calibration của mô hình. §3.5 lượng hoá generalization gap so với ASVspoof 2019 LA (Bảng 3.2) và làm nổi rằng ASV21 DF không đại diện cho codec robustness thực tế: hai mô hình XLS-R chỉ tăng khoảng +2.5 pp trên ASV21 DF nhưng tăng +18.43 đến +21.13 pp trên ASV5. §3.6 tổng hợp năm phát hiện chính (F1 codec dominance, F2 dịch chuyển phân phối codec, F3 attack tag thô không giải thích hết lỗi nhưng attack ID vẫn quan trọng, F4 fusion signal qua failure overlap/correlation, F5 front-end/back-end/checkpoint recipe đều quan trọng) và sáu giới hạn (LFCC project-specific, checkpoint công khai của AASIST3 bị loại khỏi phân tích chính, pretrained-only, EER mode theo nhóm, label noise ITW, chưa có min t-DCF/Brier/Cllr/actual DCF). Các kết quả này được dùng ở Chương 4 như pilot evidence để xây roadmap mở rộng bản đồ dữ liệu/kiến trúc và lựa chọn baseline sau khi benchmark đủ rộng hơn.

---

## Chương 4: Kết luận và Hướng phát triển

### 4.1 Tổng kết Thực tập 1 và giới hạn của pilot benchmark

Báo cáo Thực tập 1 được xem như bước khởi đầu của một mục tiêu dài hơn: xây dựng một bản đồ tương đối toàn diện cho Speech Deepfake Detection (SDD), cả về dữ liệu lẫn kiến trúc. Trong phạm vi hiện tại, báo cáo đã thực hiện ba khối công việc chính: (i) hệ thống hoá literature theo bài toán, dataset, kiến trúc và metric; (ii) xây dựng pipeline đánh giá thống nhất cho nhiều dataset; và (iii) tái lập benchmark chính với năm mô hình đại diện (LFCC+LCNN, AASIST, AASIST-L, XLS-R+AASIST, XLS-R+Nes2Net) trên bốn dataset (ASVspoof 2019 LA, ASVspoof 2021 DF, ASVspoof 5 Track 1 và In-the-Wild). AASIST3 được khảo sát ở mức checkpoint công khai nhưng không đưa vào phân tích chính vì model card Hugging Face cho biết checkpoint hiện tại đã deprecated và không phải weights dùng trong kết quả paper, như đã nêu ở §3.1.

Các kết quả reproduce trong Chương 3 có giá trị như một **pilot benchmark**. Chúng cho thấy pipeline hiện tại đã đủ để chạy nhiều checkpoint, lưu score thống nhất, tính EER cross-dataset và thực hiện error analysis theo metadata của ASVspoof 5. Đồng thời, các kết quả này tạo ra một số bài học ban đầu:

- **Thứ hạng mô hình không ổn định giữa các dataset.** XLS-R+Nes2Net dẫn đầu trên ASV19 LA, ASV21 DF và In-the-Wild, nhưng trên ASV5 lại kém XLS-R+AASIST. Điều này cho thấy một benchmark đơn lẻ không đủ để đánh giá robustness.
- **SSL front-end hiệu quả nhưng không đủ.** Hai mô hình XLS-R nhìn chung tốt hơn nhóm không dùng SSL front-end trên các dataset khó, nhưng vẫn có EER cao trên ASV5. Sự khác biệt giữa XLS-R+AASIST và XLS-R+Nes2Net cũng cho thấy back-end, checkpoint và recipe đánh giá vẫn quan trọng.
- **ASVspoof 5 cho thấy dạng lỗi theo codec/encoding.** Với hai mô hình XLS-R, subset `nocodec` có EER thấp hơn nhiều so với C04/C07, gợi ý rằng codec robustness là một trục đánh giá cần được tách riêng.
- **Failure overlap và score correlation gợi ý khả năng fusion**, nhưng đây mới là bằng chứng gián tiếp; cần thí nghiệm calibration/fusion riêng trước khi kết luận.

Tuy vậy, pilot benchmark hiện tại chưa đủ rộng để kết luận kiến trúc nào là tốt nhất cho SDD nói chung, cũng chưa đủ để chọn ra một proposal cải thiện cụ thể. Phạm vi hiện tại mới gồm năm mô hình chính, bốn dataset, chủ yếu dùng pretrained checkpoint theo chế độ out-of-the-box, và chưa bao phủ đầy đủ các kịch bản như multilingual deepfake, replay/physical channel, partial spoof, adversarial setting, neural codec generation hoặc SASV. Vì vậy, kết quả Chương 3 nên được đọc như **các tín hiệu để định hướng câu hỏi nghiên cứu tiếp theo**, không phải cơ sở cuối cùng để chọn một hướng cải tiến duy nhất.

### 4.2 Xây bản đồ dữ liệu và kiến trúc cho SDD

Hướng phát triển quan trọng nhất sau Thực tập 1 là mở rộng từ pilot benchmark sang một **benchmark map** có cấu trúc. Mục tiêu không phải chỉ chạy thêm nhiều dataset hoặc nhiều model một cách rời rạc, mà là xây dựng một ma trận đánh giá đủ đại diện cho các trục chính của bài toán SDD: loại tấn công, điều kiện thu/phát, ngôn ngữ, codec, mức độ chỉnh sửa và lựa chọn kiến trúc.

**Bản đồ dữ liệu.** Các dataset tiếp theo nên được chọn theo vai trò của chúng trong landscape thay vì theo mức độ phổ biến đơn thuần. Một bản đồ dữ liệu hợp lý cần bao phủ các nhóm sau:

- **Clean LA / TTS / VC benchmarks:** ASVspoof 2019 LA, WaveFake [33], FoR [34] hoặc các dataset lịch sử tương tự. Nhóm này giúp kiểm tra hiệu năng trong điều kiện kiểm soát và tạo baseline so sánh với literature.
- **Codec, transmission và neural codec generation:** ASVspoof 2021 DF, ASVspoof 5, CodecFake/CodecFake+. Nhóm này kiểm tra liệu detector có còn ổn định khi audio bị encode/transcode hoặc được sinh qua neural codec pipeline.
- **In-the-wild / social media:** In-the-Wild và các dataset thu thập từ nguồn công khai. Nhóm này phản ánh domain shift thực tế: codec không đồng nhất, label có thể nhiễu, speaker distribution khác và attack metadata thường thiếu.
- **Multilingual speech deepfake:** MLAAD, SpeechFake hoặc các benchmark đa ngôn ngữ khác. Nhóm này cần thiết để tránh kết luận dựa trên English-centric data.
- **Replay / physical channel:** ASVspoof PA, EchoFake và các dataset có yếu tố thiết bị/phòng/microphone. Nhóm này kiểm tra tương tác giữa spoofing và channel vật lý.
- **Partial spoof, adversarial và SASV:** PartialSpoof, ADD-style tasks, ASVspoof 5 Track 2 hoặc các adversarial track. Nhóm này mở rộng khỏi binary utterance-level CM sang localization, speaker verification-aware evaluation và robustness dưới tấn công có chủ đích.

**Bản đồ kiến trúc.** Song song với dữ liệu, cần mở rộng tập mô hình theo các quyết định thiết kế chính:

- **Hand-crafted spectral front-end:** LFCC/CQCC/MFCC + LCNN, ResNet hoặc TDNN. Đây là nhóm baseline nhẹ, hữu ích để đo mức độ phụ thuộc vào spectral artifacts.
- **Raw waveform compact models:** RawNet2, AASIST, AASIST-L và các biến thể end-to-end nhỏ. Nhóm này đại diện cho hướng học trực tiếp từ waveform nhưng không dựa vào SSL pretraining lớn.
- **SSL/foundation front-end + anti-spoofing back-end:** wav2vec2, XLS-R, WavLM, HuBERT [35] hoặc Whisper-style encoder [27] kết hợp với AASIST, MFA, Nes2Net hoặc pooling/MLP. Nhóm này hiện là hướng mạnh nhưng chi phí cao và phụ thuộc checkpoint/recipe.
- **Back-end hiện đại:** Nes2Net-LA [31], XLSR-Mamba [30], Conformer/Transformer variants, state-space models hoặc local-attention designs. Nhóm này giúp tách vai trò của representation và temporal/back-end modeling.
- **System-level methods:** score fusion, calibration, ensemble, data augmentation và domain adaptation. Đây là tầng thiết kế cần thiết nếu mục tiêu cuối là hệ thống robust, không chỉ một backbone đơn lẻ.

Kết quả mong muốn của giai đoạn này là một ma trận model × dataset đủ rộng để trả lời các câu hỏi: mô hình nào ổn định trên nhiều threat model, nhóm kiến trúc nào nhạy với codec hoặc language shift, checkpoint nào reproducible, và trục dữ liệu nào đang là điểm mù của các detector hiện tại.

### 4.3 Protocol đánh giá và tiêu chí lựa chọn baseline

Để bản đồ trên có giá trị khoa học, phần mở rộng cần đi kèm một protocol đánh giá nhất quán. Trước hết, mọi dataset nên được đưa về cùng quy ước score và label: score cao hơn nghĩa là bonafide hơn, label `bonafide = 1`, `spoof = 0`, và EER được tính cùng một cách. Với các dataset có metadata, báo cáo không chỉ EER tổng mà còn grouped EER theo attack, codec, language, speaker, channel hoặc manipulation type nếu thông tin này tồn tại.

Về lưu trữ kết quả, pipeline nên tiếp tục dùng format thống nhất như `results.pkl`, nhưng cần mở rộng metadata để phục vụ reproducibility: tên dataset/split, số utterance, checksum hoặc version nếu có, mode tính EER, model identifier, checkpoint source, ngày chạy và ghi chú về training data nếu biết. Trường hợp AASIST3 trong báo cáo này cho thấy cần tách rõ **paper architecture** và **public checkpoint behavior**: một checkpoint public bị deprecated hoặc không trùng paper weights không nên được dùng để kết luận về năng lực của kiến trúc.

Khi bản đồ đủ rộng hơn, baseline cho Thực tập 2 hoặc Luận văn nên được chọn theo nhiều tiêu chí cùng lúc:

- **Hiệu năng tổng thể:** EER trung bình tốt trên nhiều dataset, không chỉ một benchmark sạch.
- **Độ ổn định:** không suy giảm quá mạnh trên các nhóm dữ liệu như codec, in-the-wild, multilingual hoặc replay.
- **Tính khả lập:** checkpoint rõ nguồn, không deprecated, có thể chạy lại bằng pipeline thống nhất.
- **Chi phí tính toán:** phù hợp với tài nguyên thực nghiệm hiện có, đặc biệt nếu cần fine-tune hoặc ablation.
- **Failure mode rõ:** có đủ metadata/error analysis để hiểu mô hình sai ở đâu và vì sao.

Theo pilot benchmark hiện tại, XLS-R+Nes2Net và XLS-R+AASIST là hai baseline ứng viên đáng theo dõi vì chúng có hiệu năng tốt hơn phần lớn mô hình còn lại trên các dataset khó. Tuy nhiên, chưa nên chọn ra một backbone cuối cùng chỉ dựa trên bốn dataset hiện tại. Việc chọn baseline nên diễn ra sau khi benchmark map được mở rộng và sau khi các checkpoint mới được kiểm tra tính reproducible.

### 4.4 Hướng cải thiện ở mức ý niệm và kế hoạch tiếp theo

Dựa trên pilot benchmark hiện tại, có thể đề xuất một số hướng cải thiện ở mức ý niệm, nhưng các hướng này cần được xem là **hướng ứng viên** để kiểm chứng sau khi bản đồ dữ liệu/kiến trúc rộng hơn. Không nên coi chúng là kết luận cuối cùng của Thực tập 1.

- **Codec-aware training và consistency regularization.** Kết quả ASV5 cho thấy C04/C07 làm EER của nhóm XLS-R tăng mạnh, vì vậy codec-aware augmentation hoặc score-level codec consistency là một hướng đáng thử. Tuy nhiên, hướng này cần được kiểm chứng trên nhiều codec/dataset hơn, đặc biệt với held-out codec hoặc CodecFake-style data, trước khi kết luận là giải pháp chính.
- **Attack-aware hard mining.** Phân tích confident FP cho thấy A28 là FP bottleneck rõ nhất của hai mô hình XLS-R, còn A17/A26 là nhóm attack khó của họ AASIST. Do đó, ngoài codec-aware augmentation, training/curriculum nên có cơ chế mining các attack ID khó như A28 và A17 để tránh tối ưu quá mức cho tag attack thô hoặc codec condition.
- **Score-level fusion và calibration.** Failure overlap/correlation giữa các họ mô hình không hoàn toàn trùng nhau, gợi ý fusion có thể hữu ích. Nhưng fusion chỉ có ý nghĩa nếu đi kèm calibration nghiêm túc trên dev split và đánh giá cả EER lẫn calibration metrics.
- **Front-end/back-end ablation.** Kết quả Chương 3 cho thấy SSL front-end mạnh không tự đảm bảo robustness. Một hướng cần làm là giữ cùng front-end nhưng thay back-end, hoặc giữ back-end nhưng thay front-end, để tách đóng góp của representation, temporal modeling và checkpoint recipe.
- **Cross-domain và multilingual adaptation.** Nếu mở rộng sang MLAAD/SpeechFake/In-the-Wild-like data, cần kiểm tra liệu detector học dấu hiệu phụ thuộc tiếng Anh hoặc dataset source hay học được dấu hiệu spoof tổng quát.
- **Replay, partial spoof, adversarial và SASV.** Các trục này mở rộng khỏi binary utterance-level SDD hiện tại, nhưng cần thiết để tiến gần deployment thực tế hơn.
- **Phân tích định tính lỗi.** Các confident errors như trong `hard_errors.csv` nên được nghe và soi spectrogram để hiểu dấu hiệu âm học nào bị bỏ sót; phần này bổ sung cho EER và grouped metrics.

Kế hoạch tiếp theo có thể chia thành hai giai đoạn. Trong **Thực tập 2**, trọng tâm nên là mở rộng benchmark map: hoàn thiện parser/metadata cho nhiều dataset hơn, thêm các mô hình hiện đại có checkpoint reproducible, chạy lại pipeline thống nhất, xây bảng so sánh theo các trục dữ liệu/kiến trúc và chọn baseline cuối cùng dựa trên bản đồ đã mở rộng. Sau đó, trong **Luận văn**, hiện thực một hướng cải thiện cụ thể trên baseline đã chọn, tối ưu training/inference recipe và đánh giá bằng ablation study đầy đủ. Theo cách này, đề xuất cải tiến cuối cùng sẽ xuất phát từ một bức tranh thực nghiệm đủ rộng, còn phần Luận văn tập trung vào hiện thực và kiểm chứng cải tiến.

Tóm lại, đóng góp chính của Thực tập 1 không nằm ở việc chọn một phương pháp mới, mà ở việc đặt nền móng cho một bản đồ SDD có hệ thống: pipeline reproduce, benchmark ban đầu, các dạng lỗi sơ bộ và tiêu chí mở rộng. Đây là bước cần thiết để Thực tập 2 có thể chọn baseline trên cơ sở rộng hơn, trước khi Luận văn phát triển một cải tiến kiến trúc hoặc training recipe có cơ sở vững hơn.
...
[32] lab260, "AASIST3," Hugging Face model card, https://huggingface.co/lab260/AASIST3, accessed May 10, 2026.

[33] J. Frank and L. Schönherr, "WaveFake: A data set to facilitate audio deepfake detection," arXiv:2111.02813, 2021.

[34] R. Reimao and V. Tzerpos, "FoR: A dataset for synthetic speech detection," *International Conference on Speech Technology and Human-Computer Dialogue (SpeD)*, 2019.
