...
2. **Cross-dataset benchmark sáu mô hình × bốn dataset** (§3.2–§3.3, §3.5). Trong số sáu mô hình có đại diện hand-crafted (LFCC+LCNN), end-to-end nhỏ (AASIST/AASIST-L), SSL-based 2024-style (AASIST3) và hai representative SSL systems dùng XLS-R 300M (XLS-R+AASIST, XLS-R+Nes2Net). Bốn dataset bao quát clean benchmark (ASV19 LA), codec-stressed (ASV21 DF), modern-attack (ASV5 Track 1) và in-the-wild domain shift (ITW).
...
**3.4.4 Confident errors.** File `hard_errors.csv` lưu một tập top confident errors và 25 mẫu `_universal` bị cả sáu mô hình sai; trong bản export hiện tại, artifact này phù hợp cho qualitative inspection hơn là thống kê định lượng theo từng mô hình (300 dòng đầu có cột model trống và phần này đọc như confident errors của AASIST do cách sort/wide-format). Quan sát định tính cho thấy nhiều confident error là *false positive* — spoof bị cho score bonafide rất cao — đặc biệt ở các attack A26/A17/A28; đây là loại lỗi nguy hiểm trong deployment vì audio giả bị chấp nhận như thật. Phần này được dùng để chọn mẫu nghe/soi spectrogram ở Thực tập 2, không rút kết luận định lượng theo attack hay codec.
