...
Phần còn lại của báo cáo được tổ chức như sau. **Chương 2** trình bày cơ sở lý thuyết, bao gồm phát biểu bài toán, các loại tấn công, các tập dữ liệu benchmark, các nhóm kiến trúc tiếp cận, và phương pháp đánh giá. **Chương 3** mô tả thiết lập thí nghiệm, trình bày kết quả EER tổng hợp trên năm mô hình trong benchmark chính và bốn dataset, đồng thời giải thích trường hợp checkpoint AASIST3 bị loại khỏi phân tích chính. **Chương 4** tổng kết pilot benchmark, đề xuất hướng xây bản đồ dữ liệu/kiến trúc cho SDD, và nêu các candidate directions ở mức ý niệm cho Thực tập 2 và Luận văn.
...
**3.4.4 Confident errors.** Để kiểm tra định tính các lỗi tự tin cao, báo cáo soi trực quan một số audio bằng waveform, LTAS, STFT spectrogram, log-mel spectrogram, LFCC-style features và các descriptor phổ. Vì mục tiêu chính của chương là giải thích codec gap của nhóm XLS-R, phần này tập trung vào `XLS-R+Nes2Net` trên C04/C07 — hai codec làm EER của mô hình này tăng lên 41-42% ở §3.4.3.

Hai mẫu tiêu biểu là `E_0002108564` (bonafide, C04) và `E_0001333311` (bonafide, C07). Cả hai đều là false negative của `XLS-R+Nes2Net`: score bonafide rất thấp (lần lượt khoảng \(3.0\times 10^{-7}\) và \(1.3\times 10^{-5}\)), tương ứng với confidence sai gần 1. Trong các panel trực quan, đường LTAS của hard error và bonafide reference cùng codec gần như chồng khít; STFT/mel vẫn thể hiện harmonic structure và formant tương đối tự nhiên; không có cue spoof rõ ràng bằng mắt ở phổ vĩ mô. Quan sát này củng cố nhận định rằng lỗi lớn ở C04/C07 không đến từ attack family cụ thể — vì các mẫu này là bonafide — mà nhiều khả năng liên quan tới codec footprint của Encodec hoặc MP3+Encodec làm lệch representation hoặc calibration của mô hình. Đây vẫn là phân tích định tính trên một số mẫu đại diện, dùng để hỗ trợ F1/F2 chứ không thay thế grouped EER.

![Hình 3.3. Audio panel của mẫu `E_0002108564` — bonafide C04 bị `XLS-R+Nes2Net` phân loại nhầm thành spoof với score bonafide khoảng \(3.0\times 10^{-7}\).](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0002108564.png)

Ở mẫu C04 này, waveform và spectrogram cho thấy speech liên tục với harmonic structure rõ. Đường LTAS của hard error và bonafide reference cùng codec gần như chồng nhau trên toàn dải 0-8 kHz, chỉ khác nhẹ ở một số vùng tần số. Không có dấu hiệu spoof rõ ở phổ vĩ mô, nhưng model vẫn gán score bonafide gần 0. Điều này gợi ý rằng Encodec có thể tạo footprint làm representation của bonafide lệch khỏi vùng mà `XLS-R+Nes2Net` xem là bonafide.

![Hình 3.4. Audio panel của mẫu `E_0001333311` — bonafide C07 bị `XLS-R+Nes2Net` phân loại nhầm thành spoof với score bonafide khoảng \(1.3\times 10^{-5}\).](../results/viz/asv5_hard_error_visualization/plots/audio_panel_E_0001333311.png)
...
![Hình 3.5. Tương quan score giữa các mô hình trên ASVspoof 5 Track 1 eval. Hình hiển thị cả AASIST3 do là artifact thô đã chạy trước, nhưng các con số liên quan tới AASIST3 không được dùng trong diễn giải vì checkpoint Hugging Face công khai không trùng weights paper (xem §3.1).](../results/error_analysis/asvspoof5/plots/score_correlation.png)

Tóm lại, ASVspoof 5 eval xác nhận ba điểm chính: (i) tất cả mô hình đều có generalization gap lớn so với ASV19 LA; (ii) với nhóm XLS-R, EER cao trên ASV5 chủ yếu đến từ một số codec khó như C04/C07 hơn là từ toàn bộ tập attack; (iii) failure overlap và score correlation không hoàn toàn trùng nhau giữa các họ mô hình. Các quan sát này tạo motivation cho những candidate directions ở §4.4, nhưng chưa đủ để chốt một đề xuất cải thiện cụ thể.
...
- **SSL front-end giúp nhưng không đủ.** Hai mô hình XLS-R nhìn chung tốt hơn nhóm không dùng SSL front-end trên các dataset khó, nhưng vẫn có EER cao trên ASV5. Sự khác biệt giữa XLS-R+AASIST và XLS-R+Nes2Net cũng cho thấy back-end, checkpoint và recipe đánh giá vẫn quan trọng.
- **ASVspoof 5 phơi bày failure mode theo codec/encoding.** Với hai mô hình XLS-R, subset `nocodec` có EER thấp hơn nhiều so với C04/C07, gợi ý rằng codec robustness là một trục đánh giá cần được tách riêng.
...
Dựa trên pilot benchmark hiện tại, có thể đề xuất một số hướng cải thiện ở mức ý niệm, nhưng các hướng này cần được xem là **candidate directions** để kiểm chứng sau khi bản đồ dữ liệu/kiến trúc rộng hơn. Không nên coi chúng là kết luận cuối cùng của Thực tập 1.
...
Kế hoạch tiếp theo có thể chia thành hai giai đoạn. Trong **Thực tập 2**, trọng tâm nên là mở rộng benchmark map: hoàn thiện parser/metadata cho nhiều dataset hơn, thêm các mô hình hiện đại có checkpoint reproducible, chạy lại pipeline thống nhất và xây bảng so sánh theo các trục dữ liệu/kiến trúc. Sau đó, trong **luận văn**, chọn baseline dựa trên bản đồ đã mở rộng, thiết kế một hướng cải thiện cụ thể và đánh giá bằng ablation đầy đủ. Theo cách này, đề xuất cải tiến cuối cùng sẽ xuất phát từ một bức tranh thực nghiệm đủ rộng, thay vì từ một pilot benchmark còn hẹp.

Tóm lại, đóng góp chính của Thực tập 1 không nằm ở việc chốt ngay một phương pháp mới, mà ở việc đặt nền móng cho một bản đồ SDD có hệ thống: pipeline reproduce, benchmark ban đầu, các failure mode sơ bộ và tiêu chí mở rộng. Đây là bước cần thiết trước khi chọn baseline và phát triển một cải tiến kiến trúc hoặc training recipe có cơ sở vững hơn.
